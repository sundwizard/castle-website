...
class EditBlogComponent extends Component
{
    ...
    public function mount($id){
        $this->selBlog = $selBlog = Blog::find($id);
        $this->title = $selBlog->blog_title;
        $this->description = $selBlog->description;
    }
    ...
    public function updateBlog($formData)
    {
        ...
        if($this->photo){
            ...
            unlink('assets/images/blogs/'.$this->selBlog->image);
            if ($this->isImage($fileExtension)) {
            ...
            $this->selBlog->update(['image'=>$blogImageName ]);
        ...
        return redirect()->route('blogs.index')->with('feedback','Blog post created successfully');
    }

    public function render()
    {
        return view('livewire.website-admin.blogs.edit-blog-component')->layout('livewire.website-admin.layouts.app');
    }
}
